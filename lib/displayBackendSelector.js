...
class DisplayBackendSelector {
	 ...
	 * @param {{ debug?: (message: string) => void, warn?: (message: string) => void }} [log] - Adapter logger
	 */
	 ...
	 */
	async select(host) {
		if (host.displayBackend !== 'auto') {
			if (host.displayBackend === 'ddcutil') {
				const available = await new DDCUtilBackend(this.sshClient, this.log).detect();
				return available ? 'ddcutil' : undefined;
			}

			return host.displayBackend;
		}
