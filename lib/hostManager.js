...
class HostManager {
	...
	constructor(adapter) {
		...
		/** @type {Map<string, 'vcgencmd' | 'ddcutil' | 'x11' | 'custom'>} */
		this.displayBackends = new Map();
	}
	...
	async start() {
		for (const host of this.adapter.config.hosts || []) {
			...
			await createHostStates(this.adapter, host);
	...
	async testHost(host) {
		...
		try {
			...
			const backend = await new DisplayBackendSelector(sshClient, this.adapter.log).select(host);

			if (backend) {
				this.displayBackends.set(host.id, backend);
				this.adapter.log.info(`Using display backend: ${backend === 'x11' ? 'xset' : backend}`);
			} else {
				this.displayBackends.delete(host.id);
				this.adapter.log.info('Display backend: none');
			}
		} catch (error) {
			this.displayBackends.delete(host.id);
	...
	async testConnection(host) {
		if (!host || typeof host !== 'object') {
			return { success: false, error: 'Host configuration is required' };
		}
	...
	async setDisplayState(host, enabled) {
		const displayBackend = this.displayBackends.get(host.id);

		if (!displayBackend) {
			this.adapter.log.warn(`Could not set display state for host ${host.id}: no display backend available`);
			return;
		}
		...
		try {
			...
			if (displayBackend === 'ddcutil') {
				const backend = new DDCUtilBackend(sshClient, this.adapter.log);
				const success = enabled ? await backend.turnOn() : await backend.turnOff();

				if (!success) {
					throw new Error('ddcutil display command failed');
				}
			} else {
				const command = this.displayDriver.getCommand({ ...host, displayBackend }, enabled);
